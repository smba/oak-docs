...
% Introduction example for symbolic execution:
\begin{figure}
\begin{lstlisting}
<?php
$x = $_POST['name'];
...
\end{lstlisting}
\caption{Example for symbolic execution of PHP code\label{fig:symbolic}}
\end{figure}

\subsection{Workflow And Existing Tools} 
...
\begin{figure}[t!]
	\centerline{
	\includegraphics[width=0.45\textwidth]{images-paper/workflow}
	}
	\vspace{2mm}
	\caption{
		Overview of the workflow for tools based on static output approximation for dynamic web applications.	
	}
	\label{fig:approach}
\end{figure}
...
\item \emph{WebSlice}: Program slices enable to extract and understand the impact of changes in an application. 
To consider all of a dynamic web application for program slices, client page output needs to be taken into account. WebSlice combines PHP data-flow information with an output approximation to enable program slices across different languages \cite{Nguyen:2015:CPS:2786805.2786872}.

\item \emph{Varis}: Editor services such as “jump to declaration” are nontrivial for dynamic web applications due to their staged nature. Varis provides editor services across stages for client-side code: Starting with an ouput approximation a callgraph with conditional edges is constructed, which allows navigation in client-site code (HTML, JS and CSS) although it is embedded in server-side code \cite{Nguyen:2015:VIS:2819009.2819140,Nguyen:2014:BCG:2635868.2635928}.
\end{compactitem}

\subsection{Assumptions and Limitations}
\textcolor{blue}{
\begin{compactitem}
	\item Design Decisions: Single loop and recursion execution due to conceptual limitations
	\item Conceptual Limitations: Symbolic input as form input, interaction
	\item Language-related or implementation-specific: No OOP, array support, standard lib
\end{compactitem}
}
