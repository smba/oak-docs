...
% removes copyright & conference info
...
\title{Output Approximation for PHP at Scale:\\ Experience and Analysis}
...
well as analyses for vulnerability detection. However, previous tools that
leverage symbolically approximated output have only been evaluated for smaller
systems and scalability for larger systems has not been assessed.
...
such as client page output. The latter code, although generated at
...
\cite{phpsync}, computing program slices across
...
\cite{Nguyen:2015:VIS:2819009.2819140,Nguyen:2014:BCG:2635868.2635928,Nguyen:2015:CPS:2786805.2786872,phpsync}
...
symbolic execution engine, we re-implement the symbolic engine with the
...
extend it with additional features, such as object-oriented programming. We
...
face a trade-off between accuracy and scalability: Whilst exploring multiple
paths, for method invocations the number of target objects can grow rapidly and
become infeasible. Moreover, based on observations we identify limitations of
symbolic execution for PHP. For an accurate execution dynamic features, such as
dynamic function invocations and dynamic includes, require precisely resolved
include targets and function names respectively. Since these expressions  can
contain symbolic values at run-time, for many cases this restricts a symbolic
execution engine from further execution as callable code sections become
inaccessible. 
...
we criticize as well as current conceptual and language-related limitations.
...
\cite{phpsync,Nguyen:2014:BCG:2635868.2635928,Nguyen:2015:CPS:2786805.2786872,Nguyen:2015:VIS:2819009.2819140}.
...
Therefore, for our execution engine to be sound, we are bound to execute
...
that, in many cases, we were not able to execute accurately. Dynamic features in
PHP are features that take as input rather a expression that is resolved at run-time
...
expression evaluates to represents an absolute or relative path to the target
...
we borrowed three systems from a case study by Hills et al.
\cite{hills2015variable} addressing the feature usage patterns and frequencies in PHP systems. Some more detailed description of this
case study can be found in Section \ref{sec:related_work}. Many of the older small-scale systems in our corpus have been studied before in similar contexts. For instance, Nguyen et al.~\cite{phpsync,Nguyen:2014:BCG:2635868.2635928} have selected among others these systems from \sf{sourceforge.net} while considering different sizes and domains as well as little use of object-oriented constructs. For our choice of modern small-scale systems we
...
corresponding function is never invocated at run-time or simply just dead code.
...
From our observations with \sf{WordPress} in Section \ref{sec:3} we learn that
...
this goal \cite{artzi_finding_2008,artzi-tse10,DynamicWassermann}
...
\cite{phpsync,Nguyen:2014:BCG:2635868.2635928,Nguyen:2015:CPS:2786805.2786872,Nguyen:2015:VIS:2819009.2819140,minamide_static_2005,wassermann2007sound}
...
\cite{minamide_static_2005,Nguyen:2014:BCG:2635868.2635928,wang-fse12} or propose tools
based on respective approximations
\cite{phpsync,Nguyen:2014:BCG:2635868.2635928,Nguyen:2015:CPS:2786805.2786872,Nguyen:2015:VIS:2819009.2819140,wassermann2007sound,wassermann_static_2008}.
...
to increase the practicality of the tools described above.
...
combination with symbolic values can hinder precise symbolic execution
...
\bibliography{bibliography,dblp2_short,tien} 
